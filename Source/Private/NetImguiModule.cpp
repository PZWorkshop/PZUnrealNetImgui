...
//=================================================================================================
void AddFontGroup(FString name, float pxSize, float dpiScale, const uint32_t* pFontData, uint32_t FontDataSize, bool extraIconGlyphs, bool appendJapanese=false, const ImWchar* pGlyphRange=nullptr )
{
	ImFontConfig Config;
	...
	pFontAtlas->AddFontFromMemoryCompressedTTF(pFontData, FontDataSize, pxSizeScaled, &Config, pGlyphRange);
	
	Config.MergeMode = true;
#if NETIMGUI_FONT_JAPANESE
	if (appendJapanese) {
		Config.RasterizerMultiply = 1.5f;		// Boost kanji color intensity slightly, making them more readable
		pFontAtlas->AddFontFromMemoryCompressedTTF(IPAexMincho_compressed_data,	IPAexMincho_compressed_size, pxSizeScaled, &Config, pFontAtlas->GetGlyphRangesJapanese());
	...
	if( extraIconGlyphs ){
		...
		pFontAtlas->AddFontFromMemoryCompressedTTF(KenneyIcon_compressed_data, KenneyIcon_compressed_size, pxSizeScaled, &Config, iconKenney_ranges);
#endif
#if NETIMGUI_FONT_ICON_AWESOME
		...
		pFontAtlas->AddFontFromMemoryCompressedTTF(fa_solid_900_compressed_data, fa_solid_900_compressed_size, pxSizeScaled, &Config, iconFontAwesome_ranges);
		pFontAtlas->AddFontFromMemoryCompressedTTF(fa_regular_400_compressed_data, fa_regular_400_compressed_size, pxSizeScaled, &Config, iconFontAwesome_ranges);
		pFontAtlas->AddFontFromMemoryCompressedTTF(fa_brands_400_compressed_data, fa_brands_400_compressed_size, pxSizeScaled, &Config, iconFontAwesomeBrands_ranges);
#endif
#if NETIMGUI_FONT_ICON_MATERIALDESIGN
		static const ImWchar iconMaterialDesign_ranges[] = { ICON_MIN_MD, ICON_MAX_MD, 0 };
		pFontAtlas->AddFontFromMemoryCompressedTTF(MaterialIcons_Regular_compressed_data, MaterialIcons_Regular_compressed_size, pxSizeScaled, &Config, iconMaterialDesign_ranges);
#endif
	}
}
...
//=================================================================================================
void FNetImguiModule::BuildFont(float fontDPIScale)
{
	ImGuiContext* savedContext	= ImGui::GetCurrentContext();
	ImGui::SetCurrentContext(mpContext);
	ImFontAtlas* fontAtlas		= ImGui::GetIO().Fonts;
	bool needBuild				= mFontDPIScale <= 0.f || !fontAtlas->IsBuilt();

	// Detect if the new DPI change the pixel size of any of our font
	for (int i(0); !needBuild && i < fontAtlas->Fonts.size(); ++i) {
		int pixelSizeNative	= static_cast<int>(fontAtlas->Fonts[i]->FontSize / mFontDPIScale);
		int pixelSizeNeeded	= static_cast<int>(static_cast<float>(pixelSizeNative) * fontDPIScale);
	...
	if( needBuild )
	{
		...
		fontConfig.SizePixels = 13.f*mFontDPIScale;
		fontAtlas->AddFontDefault(&fontConfig);	// Proggy Clean

		AddFontGroup(TEXT("Cousine Fixed"),		16.f, mFontDPIScale, Cousine_Regular_compressed_data,	Cousine_Regular_compressed_size,	true, true);
		AddFontGroup(TEXT("Cousine Fixed"),		20.f, mFontDPIScale, Cousine_Regular_compressed_data,	Cousine_Regular_compressed_size,	true, true);
		AddFontGroup(TEXT("Cousine Fixed"),		24.f, mFontDPIScale, Cousine_Regular_compressed_data,	Cousine_Regular_compressed_size,	true, true);
		AddFontGroup(TEXT("Karla Regular"),		16.f, mFontDPIScale, Karla_Regular_compressed_data,		Karla_Regular_compressed_size,		true);
		AddFontGroup(TEXT("Droid Sans"),		20.f, mFontDPIScale, Droid_Sans_compressed_data,		Droid_Sans_compressed_size,			true);
		AddFontGroup(TEXT("Proggy Tiny"),		10.f, mFontDPIScale, Proggy_Tiny_compressed_data,		Proggy_Tiny_compressed_size,		false);
		AddFontGroup(TEXT("Roboto Medium"),		16.f, mFontDPIScale, Roboto_Medium_compressed_data,		Roboto_Medium_compressed_size,		true);
		AddFontGroup(TEXT("Icons"),				32.f, mFontDPIScale, Cousine_Regular_compressed_data,	Cousine_Regular_compressed_size,	true);
		AddFontGroup(TEXT("Icons"),				64.f, mFontDPIScale, Cousine_Regular_compressed_data,	Cousine_Regular_compressed_size,	true);
	#if NETIMGUI_FONT_JAPANESE
		AddFontGroup(TEXT("日本語"),				32.f, mFontDPIScale, IPAexMincho_compressed_data,		IPAexMincho_compressed_size,		true, false, fontAtlas->GetGlyphRangesJapanese());
	#endif
